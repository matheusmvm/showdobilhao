...
let audioCtx=null, timerId=null, timerMax=0, nextLock=false;
let state={index:0,correct:0,errors:0,lives:3,prize:0,streak:0,best:0,gender:'masculino',avatar:'masculino',avatarState:'normal',name:'Jogador',topic:'Conhecimentos Gerais',used50:false,usedSkip:false,usedAudience:false,usedExpert:false,usedSwap:false,usedTime:false,answered:false,finished:false,questions:[],timeLeft:0};
...
function renderTopics(){const box=$('topicGrid');if(!box)return;const entries=Object.entries(TOPICS);box.innerHTML=entries.map(([k,v])=>{const cult=k==='Desafio Cult';return `<button type="button" class="topic-card ${cult?'cult-topic':''}" data-topic="${escapeHtml(k)}" aria-label="${escapeHtml(v[1])}"><span class="topic-icon">${v[0]}</span><b>${v[1]}</b><small>${v[2]}</small><em>${cult?'50 perguntas difíceis • 30 na partida • 60s por pergunta':'45 perguntas no banco • 30 na partida'}</em></button>`}).join('');box.querySelectorAll('.topic-card').forEach(b=>b.onclick=()=>selectTopic(b.dataset.topic));selectTopic(state.topic);document.documentElement.dataset.topicCount=String(entries.length)}
...
function updateLives(){
 const cult=state.topic==='Desafio Cult';
 if(cult){
  $('lives').textContent='∞';
  $('lives').setAttribute('aria-label','Sem eliminação');
  $('errors').textContent='';
  $('errors').setAttribute('aria-hidden','true');
  $('lifeBar').style.width='100%';
  $('lifeBar').classList.add('cult-life');
  $('lifeNote').textContent='DESAFIO CULT • sem eliminação • resultado no final';
 }else{
  $('errors').removeAttribute('aria-hidden');
  $('lives').textContent='❤'.repeat(state.lives)+'♡'.repeat(3-state.lives);
  $('lives').setAttribute('aria-label','Vidas');
  $('errors').textContent=`ERROS ${state.errors}/3`;
  $('lifeBar').style.width=`${(state.lives/3)*100}%`;
  $('lifeBar').classList.remove('cult-life');
  $('lifeNote').textContent='3 erros encerram a partida';
 }
}
function updateLifelines(){[['hint50',state.used50],['hintAudience',state.usedAudience],['hintSkip',state.usedSkip],['hintExpert',state.usedExpert],['hintSwap',state.usedSwap],['hintTime',state.usedTime]].forEach(([id,used])=>$(id).disabled=used||state.answered);}
function makeQuestionFromRaw(topic,x,p){const opts=shuffle(x[2].map((text,i)=>({text,i})));return {topic,d:x[0],q:x[1],a:opts.map(o=>o.text),c:opts.findIndex(o=>o.i===x[3]),p};}
function replaceCurrentQuestion(){const bank=raw[state.topic]||[];const current=state.questions[state.index];const candidates=bank.filter(x=>x[1]!==current.q&&(state.topic==='Desafio Cult'||x[0]===current.d));if(!candidates.length)return false;const x=shuffle(candidates)[0];state.questions[state.index]=makeQuestionFromRaw(state.topic,x,current.p);return true;}
...
function startTimer(){
 ...
 const s=getSettings();
 const seconds=state.topic==='Desafio Cult'?60:s.timerSeconds;
 if(state.topic!=='Desafio Cult'&&!s.timer){$('timer').classList.add('off');$('timer').textContent='SEM TEMPO';return}
 timerMax=seconds;
 state.timeLeft=seconds;
 $('timer').classList.remove('off','danger');
 paintTimer();
 timerId=setInterval(()=>{
  if(state.answered||state.finished){clearInterval(timerId);return;}
  state.timeLeft=Math.max(0,state.timeLeft-1);
  paintTimer();
  if(state.timeLeft===0){clearInterval(timerId);timeoutAnswer();}
 },1000);
}
function paintTimer(){
 const base=timerMax||((state.topic==='Desafio Cult')?60:getSettings().timerSeconds);
 const pct=clamp(state.timeLeft/base*100,0,100);
...
function timeoutAnswer(){
 ...
 state.streak=0;
 state.errors++;
 if(state.topic!=='Desafio Cult')state.lives=Math.max(0,3-state.errors);
 syncStateAvatar('sad');
 const cult=state.topic==='Desafio Cult';
 react('bad',cult?`TEMPO ESGOTADO! Resposta correta: ${String.fromCharCode(65+q.c)}.`:state.errors>=3?'TEMPO ESGOTADO! 3 erros. Fim de jogo.':'TEMPO ESGOTADO! A planta estava mais rápida.');
 tone(130,.2);
 $('nextBtn').classList.remove('hidden');
 updateLifelines();updateLives();
}
function syncStateAvatar(s){state.avatarState=s;syncAvatars()}
function loadQuestion(){
 const q=state.questions[state.index];if(!q){finish();return}nextLock=false;state.answered=false;clearInterval(timerId);timerMax=0;syncStateAvatar('normal');
...
function answer(i,btn){
 ...
 state.answered=true;
 clearInterval(timerId);
 const q=state.questions[state.index],buttons=[...document.querySelectorAll('.answer')];
 buttons.forEach(b=>b.disabled=true);
 const cult=state.topic==='Desafio Cult';
 if(i===q.c){
  tone(720,.11);btn.classList.add('correct');state.correct++;state.streak++;state.best=Math.max(state.best,state.streak);state.prize=q.p;syncStateAvatar('happy');
  react('good',cult?'CERTO! Resposta correta.':REACTIONS.good[Math.floor(Math.random()*REACTIONS.good.length)]);
 }else{
  tone(180,.16);btn.classList.add('wrong');buttons[q.c]?.classList.add('correct');state.streak=0;state.errors++;
  if(!cult)state.lives=Math.max(0,3-state.errors);syncStateAvatar('sad');
  react('bad',cult?`ERRADO! Resposta correta: ${String.fromCharCode(65+q.c)}.`:state.errors>=3?'ERROU! 3 erros. Fim de jogo.':REACTIONS.bad[Math.floor(Math.random()*REACTIONS.bad.length)]);
 }
 $('explanation').textContent=q.e||'Resposta registrada. Continue para a próxima pergunta.';
 $('explanation').classList.add('show');
 $('nextBtn').classList.remove('hidden');
 $('progress').style.width=`${((state.index+1)/state.questions.length)*100}%`;
 updatePrize();updateLifelines();updateLives();
}
function startGame(){
 document.body.classList.remove('cult-result');
 if($('resultEyebrow'))$('resultEyebrow').textContent='FIM DE JOGO';
 ...
 const name=($('username').value.trim()||'Jogador').replace(/\s+/g,' ').slice(0,18);state={...state,index:0,correct:0,errors:0,lives:3,prize:0,streak:0,best:0,used50:false,usedSkip:false,usedAudience:false,usedExpert:false,usedSwap:false,usedTime:false,answered:false,finished:false,name,avatarState:'normal',questions};
...
function finish(gameOver=false){
 ...
 clearInterval(timerId);state.finished=true;tone(880,.14);
 const total=state.questions.length||30;
 const data=getRanking();
 data.push({name:state.name,score:state.prize,correct:state.correct,errors:state.errors,best:state.best,date:Date.now(),gender:state.gender,avatar:state.avatar,topic:state.topic,total});
 data.sort((a,b)=>b.score-a.score||b.correct-a.correct||b.best-a.best||b.date-a.date);setRanking(data.slice(0,50));
 $('resultPrize').textContent=`R$ ${money(state.prize)}`;
 $('resultCorrect').textContent=`${state.correct}/${total}`;
 $('resultErrors').textContent=state.topic==='Desafio Cult'?`${state.errors}`:`${state.errors}/3`;
 $('resultBest').textContent=state.best;
 if(state.topic==='Desafio Cult'){
  document.body.classList.add('cult-result');
  if($('resultEyebrow'))$('resultEyebrow').textContent='RESULTADO DO DESAFIO';
  $('resultTitle').textContent='DESAFIO CULT CONCLUÍDO';
  $('resultText').textContent=`Você completou as ${total} perguntas. Acertos: ${state.correct} • Erros: ${state.errors}.`;
 }else{
  $('resultTitle').textContent=gameOver?'3 ERROS — FIM DE JOGO':state.correct===total?'VOCÊ DOMINOU O PALCO!':state.correct>=20?'MANDOU MUITO BEM!':state.correct>=5?'BOM JOGO!':'VALEU A TENTATIVA!';
  $('resultText').textContent=gameOver?'Você chegou ao limite de 3 erros. Respira, tenta de novo e busca o milhão!':REACTIONS.end[Math.floor(Math.random()*REACTIONS.end.length)];
 }
 navigate('result');
}
...
function init(){
 ...
 $('startBtn').onclick=startGame;$('nextBtn').onclick=()=>{if(!state.answered||nextLock)return;nextLock=true;if(state.topic!=='Desafio Cult'&&state.errors>=3){finish(true);return}if(state.index>=state.questions.length-1)finish(false);else{state.index++;loadQuestion()}};
 ...
 $('hintSkip').onclick=()=>{if(state.usedSkip||state.answered)return;state.usedSkip=true;state.answered=true;clearInterval(timerId);react('good','PULOU! A próxima vem aí.');tone(330,.08);updateLifelines();setTimeout(()=>{if(state.finished)return;if(state.index>=state.questions.length-1)finish(false);else{state.index++;loadQuestion()}},450)};
 $('hintExpert').onclick=()=>{if(state.usedExpert||state.answered)return;state.usedExpert=true;const q=state.questions[state.index];const pct=Math.floor(Math.random()*12)+82;react('good',`DICA DO ESPECIALISTA: a alternativa ${String.fromCharCode(65+q.c)} aparece com ${pct}% de confiança.`);tone(520,.08);updateLifelines()};
 $('hintSwap').onclick=()=>{if(state.usedSwap||state.answered)return;state.usedSwap=true;clearInterval(timerId);if(replaceCurrentQuestion()){react('good','TROCA! Uma nova pergunta entrou no lugar desta.');tone(390,.08);loadQuestion()}else{react('bad','Não havia outra pergunta disponível.');state.usedSwap=false;updateLifelines()}};
 $('hintTime').onclick=()=>{if(state.usedTime||state.answered)return;state.usedTime=true;state.timeLeft+=20;timerMax+=20;paintTimer();tone(600,.08);react('good','TEMPO EXTRA! +20 segundos.');updateLifelines()};
